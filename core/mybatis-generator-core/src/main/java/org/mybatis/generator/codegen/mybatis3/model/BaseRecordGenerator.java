    ...
    public List<CompilationUnit> getCompilationUnits() {
        ...
        for (IntrospectedColumn introspectedColumn : introspectedColumns) {
            ...
            if (plugins.modelFieldGenerated(field, topLevelClass,
                ...
                /* 添加校验注解 */
                // FIXME - 修改包名
                topLevelClass.addImportedType(new FullyQualifiedJavaType(ShellRunner.VALIDATIONGROUP_BASE_CLASS));
                // 不为空
                if (!introspectedColumn.isNullable()){
                	topLevelClass.addImportedType(new FullyQualifiedJavaType("javax.validation.constraints.NotNull"));
					field.addAnnotation("@NotNull(message = \"" + introspectedColumn.getRemarks() + "不能为空\", groups = {CreateGroup.class,ModifyGroup.class})");
                }
                
                // 长度限制,仅对字符串生效
                if (introspectedColumn.getLength() != 0 && introspectedColumn.getJdbcTypeName().toUpperCase().contains("CHAR")){      
                	topLevelClass.addImportedType(new FullyQualifiedJavaType("javax.validation.constraints.Size"));
					field.addAnnotation("@Size(max = " + introspectedColumn.getLength()  + ", message = \"" + introspectedColumn.getRemarks() + "长度不能超过" + introspectedColumn.getLength() + "\", groups = {CreateGroup.class,ModifyGroup.class,QueryGroup.class,ConditionedQueryGroup.class})");
                }
            }
